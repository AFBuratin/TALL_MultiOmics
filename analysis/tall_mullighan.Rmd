...
leukemia_genes = list(DIABLO = unlist(diabloPanel$mRNA.expr),
...
mixOmics::circosPlot(object = diablo.plot, cutoff = 0.5,
                             line = TRUE, 
                             color.Y = c("#619CFF", "#F8766D"),
           color.blocks= c("#1F78B4", "#33A02C", "#E31A1C"),
           color.cor = c("chocolate3","grey20"), 
           size.labels = 1, comp = c(1:3), 
           size.variables = 0.001)
dev.off()
network(mat = diablo.plot, blocks = c(1,2,3), interactive = F, 
        color.node = c("#1F78B4", "#33A02C", "#E31A1C"), cutoff = 0.75)
...
enrichment <- sear(input, "mrna") %>% 
    ...
    filter(fdr < 0.1, collection %in% "C2") %>% 
