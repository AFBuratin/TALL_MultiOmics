...
```{r}
data.frame(keepX = rep(rownames(tune$error.rate), 5), 
...
varDat.train <- varDat.train %>% group_by(Class, subj) %>% 
  ...
  dplyr::select(`comp1`, `comp2`, `comp3`, `comp4`, `comp5`, subj, Dataset, Class) %>% 
  as.data.frame() %>% 
  rbind(., varDat.train) %>% 
  mutate(Dataset = factor(Dataset, c("Consensus", "mut.status", "mRNA.expr", "circRNA.expr"))) 
colnames(varDat.train) <- c("comp 1", "comp 2", "comp 3", "comp 4", "comp 5", "subj", "Dataset", "Class")
...
mixOmics::plotIndiv(diablo, ind.names = FALSE, legend = TRUE, 
          title = 'Sample Plots')
dev.off()
mixOmics::plotVar(diablo, var.names = F, 
...
cplot = mixOmics::circosPlot(object = diablo.plot, cutoff = 0.5, line = TRUE,
           color.blocks= c('darkorchid', 'brown1', 'lightgreen'),
           color.cor = c("chocolate3","grey20"), size.labels = 1.5)
cplot
network(diablo.plot, blocks = c(1,2,3), interactive = F,
        color.node = c('darkorchid', 'brown1', 'lightgreen'), cutoff = 0.7)

mixOmics::plotLoadings(diablo, comp = 1, contrib = 'max', method = 'median')
```
...
plot.new()
mixOmics::cimDiablo(diablo.plot, row.names = T, col.names = T, 
                    clust.method = c("ward.D", "ward.D"),
                    dist.method = c("correlation","correlation"),
                    row.cex = 0.6, col.cex = 0.3,
                    legend.position = "topright", margins = c(10, 10), 
                    # save = "png", name.save = "varSel2", 
                    size.legend = 1)
```
...
plot(net, edge.curved=.5, vertex.label.cex=0.5, vertex.size=8, mark.groups=mark.groups, 
  mark.col=mark.col, vertex.label.color="black") 
legend("bottomleft", names(diabloPanel)[c(2,3,1)], col=unique(V(net)$color), pch = 19, bty="n", cex = 1.5)
```
...
input <- unlist(strsplit(sapply(strsplit(mark.groups[[3]], "_"), function(i) i[2]), ";"))
